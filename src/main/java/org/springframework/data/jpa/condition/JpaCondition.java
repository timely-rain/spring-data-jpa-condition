...
public class JpaCondition<T> {
  ...
  /**
   * 获得实体类的spring-beans属性流 include by names
   *
   * @return Stream<PropertyDescriptor>
   */
  ...
  /**
   * 获得实体类的spring-beans属性流 exclude by names
   *
   * @return Stream<PropertyDescriptor>
   */
  ...
  public <T extends Comparable<? super T>> Predicate between(boolean ignoreNull, String name) {
    ...
    @SuppressWarnings("unchecked") T startValue = (T) JpaConditionUtils
        .getPropertyValue(model, start);
  ...
  /**
   * 属性条件断言, 对实体类属性尝试生成条件断言 支持 Spring Data JPA 方法引用
   *
   * @param name 属性名
...
//    }
  ...
  protected <P, V> Predicate propertyPredicate(boolean ignoreNull, PropertyDescriptor descriptor,
      BiFunction<Expression<P>, V, Predicate> function) {
